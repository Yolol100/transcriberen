...
- Publieke YouTube-bronnen zijn captions/metadata-only. Een losse video zonder bereikbare captions faalt gecontroleerd; in een kanaal/playlist blijft de video als `no_usable_captions` in het bronregister staan.
...
- `reuse_allowed` en `rights_basis` zijn expliciete requestvelden. Zonder hergebruiktoestemming wordt geen samengevoegde captiontekst als `content.md` bewaard.
...
- geen caption betekent `no_usable_captions`, niet automatisch audio downloaden.
...
De workflow uploadt `transcription-result-<request_id>` met minimaal `result.json` en toolversies. YouTube-collecties leveren daarnaast `source-register.json` en `knowledge-handoff.json`; `content.md` bestaat alleen wanneer `reuse_allowed=true`.

De repository muteert geen projectbron of Skill zelfstandig. `knowledge-handoff.json` is reviewmateriaal voor Webactueel-workflow: accepteer alleen onderbouwde inzichten, dedupliceer/parafraseer ze, routeer ieder geaccepteerd inzicht naar precies één canonieke eigenaar en pas een write alleen toe met actuele readback/hash, backup, validatie en rollback.
